...
trait AsDraftSupport {
  ...
  def mkObj(vl: ValidationList, x: `object`[_], par: ParentSchema): obj = {
    ...
    val props = ListMap.from(x.fields.map { field =>
  ...
  class ValidationList(elements: ListBuffer[V.Def[_, _]]) {
    ...
    def json: obj = obj {
      ListMap.from(elements.map { d => d.validation.name -> d.json })
    }
  }
  ...
  def inferDefinitions(x: Schema[_]): obj = {
    def extractDefs(tpe: json.Schema[_], par: ParentSchema): Seq[(String, obj)] = tpe match {
      case x: `def`[_]          =>
        println("t1")
        extractDefs(x.tpe, par) :+ inferDefinition(x, par)
      case x: `allof`[_]        =>
        println("t2")
        x.subTypes.toSeq flatMap { extractDefs(_, Some(x)) }
      case x: `oneof`[_]        =>
        println("t3")
        x.subTypes.toSeq flatMap { extractDefs(_, Some(x)) }
      case x @ `array`(ref, _)  =>
        println("t4")
        extractDefs(ref, Some(x))
      case x @`dictionary`(ref) =>
        println("t5")
        extractDefs(ref, Some(x))
      case x @ `object`(fields) =>
        println(s"t7 $fields")
        fields.toSeq map { _.tpe } flatMap { extractDefs(_, Some(x)) }
      case _                    => Seq.empty
    }

    obj {
      val ret = ListMap.from(extractDefs(x, None))
      println(s"ret = $ret")
      ret
    }
  }
