...
object AsCirce {
  ...
  final def apply(value: Value): Json = {
    value match {
      case `null` => Json.Null
      case `true` => Json.True
      ...
      case arr(x) =>
        val arr = x map AsCirce.apply; Json.arr(arr.toSeq: _*)
      case obj(x) =>
        val map = ListMap.from(x) mapV AsCirce.apply
  ...
  implicit def toValue[T](implicit w: Encoder[T]): ToValue[T] = new ToValue[T] {
    override def apply(x: T): Value = {
      ...
      def translate(js: Json): Value = js match {
        ...
        case x if x.isObject => val map = ListMap.from(x.asObject.get.toList) mapV translate; obj(map)
      }
