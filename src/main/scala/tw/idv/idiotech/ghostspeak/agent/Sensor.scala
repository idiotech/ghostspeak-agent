...
object Sensor {
  ...
  def spawnScenario(context: ActorContext[Command], message: Message): Option[ActorRef[Scenario.Command]] = ???

  def onCommand(ctx: ActorContext[Command])(state: State, cmd: Command): EffectBuilder[Event, State] = cmd match {
    case Sense(message) =>
      val scenarioId = message.scenarioId
      state.get(scenarioId).fold(
