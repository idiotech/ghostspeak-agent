...
object PerformanceLogger {
  ...
  def insert(user: String, action: String, stage: String, time: Long): Unit = {
    val shortId = user.take(5)
    sql"insert into action_events (username, action, stage, time) values ($shortId, $action, $stage, $time)".update.apply()
  }
