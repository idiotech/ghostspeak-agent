...
import tw.idv.idiotech.ghostspeak.agent.{Actuator, FcmSender, Message => BaseMessage, Scenario, Sensor, Session, SystemPayload, Action => BaseAction}
...
//  implicit val messageDecoder = BaseMessage.decoder[EventPayload]
  ...
  case class Node(name: String, trigger: Message, actions: List[Action], children: List[Node]) {
    def replace(user: String): Node = {
      Node(name, substitute(trigger, "?u", user), actions.map(substitute(_, "?u", user)), children.map(_.replace(user)))
    }
  ...
  def exampleNode(user: String) = Node(
    ...
    List(
      ...
      Node("node3", message2(user).forComparison, List(action2(user), action4(user), action5(user), action6(user)), List(
