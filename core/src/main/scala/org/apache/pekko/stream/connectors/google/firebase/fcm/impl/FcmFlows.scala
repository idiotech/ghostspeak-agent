...
private[fcm] object FcmFlows {
  ...
  private def resolveSettings(
    conf: FcmSettings
  )(mat: Materializer, attr: Attributes): GoogleSettings = {
    ...
    val credentials =
      if (conf.privateKey == "deprecated")
        ...
        Credentials.cache(
          (conf.projectId, conf.clientEmail, conf.privateKey, scopes, mat.system.name)
        ) {
    ...
    val forwardProxy =
      conf.forwardProxy
        .map(_.toCommonForwardProxy(mat.system))
        .orElse(settings.requestSettings.forwardProxy)
    settings.copy(
